...
import com.catsoft.vktinA.BuildConfig
import java.io.File
        ...
        fun openFile(context : Context, fileImagePath: File) {
            ...
            when {
                ...
                url.contains(".jpg", true) || url.contains(".jpeg", true)
                        || url.contains(".png", true) -> { // JPG file
                    intent.setDataAndType(uri, "image/jpeg")
                }
                url.contains(".txt", true) -> { // Text file
                    intent.setDataAndType(uri, "text/plain")
                }
                url.contains(".3gp", true) || url.contains(".mpg", true)
                        || url.contains(".mpeg", true) || url.contains(".mpe", true)
                        || url.contains(".mp4", true) || url.contains(".avi", true) -> { // Video files
